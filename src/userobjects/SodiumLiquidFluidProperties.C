...
void
SodiumLiquidFluidProperties::cv_from_v_e(
...
void
SodiumLiquidFluidProperties::mu_from_v_e(
    Real v, Real e, Real & mu, Real & dmu_dv, Real & dmu_de) const
{
  ...
  else
  {
    ...
    etal_t_Na(T, mu, dmudt, d2mudt2); // eta in lbm/ft-hr
...
Real
SodiumLiquidFluidProperties::T_from_p_h(Real p, Real h) const
{
  p *= _to_atm;
  h *= _to_Btu_lb;
...
void
SodiumLiquidFluidProperties::T_from_p_h(Real p, Real h, Real & T, Real & dT_dp, Real & dT_dh) const
{
  p *= _to_atm;
  h *= _to_Btu_lb;
